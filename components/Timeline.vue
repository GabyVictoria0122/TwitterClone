<template>
  <div>
    <v-toolbar
      elevation="4"
    >
      <v-toolbar-title> Página Inicial </v-toolbar-title>
      <v-spacer />
      <v-btn icon>
    ...
    <v-list three-line>
      <template v-for="(tweet, index) in tweets">
        ...
        >
          ...
          <v-list-item-content>
            <v-list-item-title>
              <nuxt-link :to="{ name: 'user-username', params: { username: tweet.username }}">
                {{ tweet.name }}  {{ tweet.username }}
              </nuxt-link>
              {{ tweet.data }} <v-btn icon>
